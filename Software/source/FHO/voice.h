...
class voice
{
    ...
    void set_fold(float fold);
    void set_distort(float distort);
    float Process();

    private:
    float freq_, fine_, fold_factor_, distort_factor_;
    Oscillator osc_;
    FIR<FIRFILTER_USER_MEMORY> filter_up_; //upsampling filter
    FIR<FIRFILTER_USER_MEMORY> filter_down_; //downsampling filter
    FIR<FIRFILTER_USER_MEMORY> filter_dist_; //distortion bandlimiting filter

    int upsample_factor_ = 2;
